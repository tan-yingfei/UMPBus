...
if(isset($_POST["approve"])) {
		...
		$sql = "UPDATE user_booking SET status = :paydes , price =:prices , remarks =:remark
            WHERE num_user_booking = :pkorder";
		$stmt = $dbo->prepare($sql);
		$stmt->bindParam(':paydes', $_POST["status"], PDO::PARAM_STR);
		...
		header('Location: adminapproval.php?orderid='.$orderid);
...
<footer>
    ...
    <div class="container text-center">
        <div class="row">
            <div class="box">
                <div class="col-lg-12">
                    ...
                    <h2 class="intro-text text-center">
                        <strong>Paperwork Approvals</strong>
                    </h2>
                    <hr>
          ...
          <br/>
			  <h1>Paperwork</h1>
              <h3><?php if (!empty($s["paper_work"])) { ?><span class="glyphicon glyphicon-edit"></span> Approval<?php } else { ?><span class="glyphicon glyphicon-edit"></span> Approval<?php } ?></h3>
              <h4><?php if ($s["status"] == "1") { ?><span style="color:green" >Status = Recieved</span><?php } elseif ($s["status"] == "0") { ?><span style="color:yellow;" >Status = Pending</span><?php } elseif ($s["status"] == "3") { ?><span style="color: red;" >Paperwork Status = Rejected</span><?php } ?></h4>
              <br/><br/>
              <?php if (!empty($s["paper_work"])) { ?>
				  
				<!-- <img src="upload/<?php echo $orderid; ?>/<?php echo $s["paper_work"];?>" class="img-thumbnail" alt="" width="304" height="236">  -->
